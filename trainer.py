...
class Trainer(object):
  ...
  def train(self):
    ...
    train_dataloader = DataLoader(
      self.train_dataset, sampler=train_sampler, batch_size=self.args.train_batch_size)  # type: ignore
    ...
    logger.info("  Num examples = %d", len(self.train_dataset))  # type: ignore
  ...
  def evaluate(self, mode):
    ...
    eval_loss = 0.0
    nb_eval_steps = 0
    intent_preds: np.ndarray = None  # type: ignore
    slot_preds: np.ndarray = None  # type: ignore
    out_intent_label_ids: np.ndarray = None  # type: ignore
    out_slot_labels_ids: np.ndarray = None  # type: ignore
