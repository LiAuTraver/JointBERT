...
def predict(pred_config):
  ...
  all_slot_label_mask: np.ndarray = None  # type: ignore
  ...
  with open(pred_config.output_file, "w", encoding="utf-8") as f:
    for words, slot_preds_, intent_pred in zip(lines, slot_preds_list, intent_preds):
      line = ""
      for word, pred in zip(words, slot_preds_):
        if pred == 'O':
          line = line + word + " "
        else:
